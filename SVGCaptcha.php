...
EOD;
     ...
     */
    public static function getInstance(int $numchars, int $width, int $height, int $difficulty = SVGCaptcha::MEDIUM): object
    {
     ...
     */
    private function __construct(int $numchars, int $width, int $height, mixed $difficulty) {
     ...
     */
    private function _shape2_cmd(array $shape, bool $absolute = True, bool $explicit_moveto = False): bool|string
    {
        if ($explicit_moveto) {
     ...
     */
    private function _scale_by_largest_glyph(array &$glyphs) {
     ...
     */
    private function on_points(mixed &$data, array $callback, array $args) {
        // Base step
     ...
     * @throws ErrorException
     */
    private function _ra($ub=null): float|int
    {
     ...
     * @throws ErrorException
     */
    private function _rt(bool $inclusive = True): float|int
    {
        if ($inclusive) {
     ...
     * @param float|int $s The scale factor for the x/y-component.
     */
    private function _scale(Point $p, float|int $s = 1) {
        $x = $p->x;
        $y = $p->y;
        $p->x = $x * $s;
        $p->y = $y * $s;
 ...
 */
function pick_remaining($key_pool, $already_picked): mixed
{
...
if (!function_exists('array_column')) {
     ...
     * @return array
     */
    function array_column(array $input = null, mixed $columnKey = null, mixed $indexKey = null): bool|array|null
    {
